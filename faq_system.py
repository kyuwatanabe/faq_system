...
"""
        ...
        prompt = f"""
以下の文章を読んで、この文章に**明確に記載されている具体的な情報**に基づいて答えられる「ビザ申請者の実際の疑問シナリオ」を全て抽出してください。
...
【タスク】
この文章から、実際にビザ申請者が遭遇しそうな問題シナリオを**できるだけ多く**抽出してください。
**14種類の質問バリエーション**を意識して、様々な切り口でシナリオを考えてください。
...
【絶対禁止シナリオ（抽出してはいけない）】
❌ 漠然とした表現：「自分の〜」「〜の方法」「どのように判断」「確認する方法」「選ぶ方法」
❌ 文書に記載されていない一般論を求めるシナリオ
❌ 複数の条件が組み合わさった複雑すぎるシナリオ

【重要ルール】
✓ 文章に**具体的に記載されている内容のみ**からシナリオを抽出すること
✓ 固有名詞（ビザ種類、書類名、制度名など）が含まれる**シンプルな**シナリオであること
✓ **14種類のバリエーションから様々な切り口のシナリオを抽出する**こと（同じ切り口ばかりにならないように）
✓ 使用禁止シナリオは除外すること
✓ **最低10個以上**抽出すること（より多い方が良い）
...
【タスク】
1. 上記のシナリオに困っているビザ申請者が実際に聞きそうな**シンプルな質問**を作成する
2. **情報源に明確に記載されている内容のみ**を使って**簡潔な回答**を作成する

【14種類の質問バリエーション】
以下のいずれかの形式で質問を作成すること：
1. 定義・概念：「〜とは何ですか？」
2. 条件・要件：「〜の条件は何ですか？」「〜は必要ですか？」
3. 手順・方法：「どのように〜しますか？」「〜の手順は？」
4. 時期・期間：「いつ〜しますか？」「〜はいつまでですか？」
5. 場所・窓口：「どこで〜できますか？」
6. 主体・責任：「誰が〜しますか？」
7. 理由・背景：「なぜ〜なのですか？」
8. 結果・影響：「〜するとどうなりますか？」
9. 例外・特殊ケース：「〜の場合はどうなりますか？」
10. 比較・選択：「〜と〜の違いは？」
11. トラブル・問題解決：「〜できない場合はどうすればよいですか？」
12. 確認・検証：「〜は含まれますか？」
13. 範囲・対象：「〜は誰が対象ですか？」
14. 費用・コスト：「〜にいくらかかりますか？」

【質問作成の要件】
✓ **実用性**: このシナリオに困っている人が実際に聞く質問であること
✓ **具体性**: 具体的なビザ種類・書類名・制度名などの固有名詞を含むこと
✓ **簡潔性**: **20-40文字程度のシンプルな質問文**にすること
✓ **完結性**: 質問単独で意味が通じること
✓ **自然さ**: 上記14種類のバリエーションのいずれかの形式を使うこと
✓ **文書依存性**: 情報源の文章に直接記載されている内容に基づいた質問であること

【回答作成の要件】
✓ **情報源に明確に記載されている情報のみ**を使用すること
✓ **簡潔性**: **必ず80-120文字以内**で要点を絞って記載すること
✓ **文字数制限厳守**: 120文字を超える回答は絶対に作成しないこと
✓ 一般論や推測を含めないこと
✓ 具体的な手続き・条件・例外などを**最も重要なもの1-2点のみ**簡潔に記載すること
✓ 「ビザの種類によって異なる」などの漠然とした表現は避けること
✓ 条件が複数ある場合は列挙せず、最も重要なものに絞ること

【絶対禁止の質問パターン】
❌ 漠然とした表現：「自分の〜」「〜の方法」「どのように判断」「確認する方法」「選ぶ方法」
❌ 文書に記載されていない一般論を求める質問
❌ 50文字を超える複雑な質問

【出力形式】
JSON配列形式で**1個だけ**出力してください：
    ...
    def generate_faqs_from_document(self, pdf_path: str, num_questions: int = 3, category: str = "AI生成") -> list:
        ...
        try:
            ...
            while len(all_faqs) < num_questions and generation_attempt < max_total_attempts:
                ...
                print(f"[DEBUG] ステップ1: シナリオ抽出開始...")

                scenarios = self._extract_scenarios(
                    window_text=window_pair['question_text'],
                    used_scenarios=window_used_scenarios
                )

                scenario_time = time.time() - api_start_time
                print(f"[TIME] シナリオ抽出時間: {scenario_time:.1f}秒")

                if not scenarios:
                    print(f"[DEBUG] 生成試行 {generation_attempt} シナリオ抽出失敗")
                    continue

                # ステップ2: Pythonでランダム選択
                import random
                selected_scenario = random.choice(scenarios)
                print(f"[DEBUG] 選択シナリオ: {selected_scenario}")
                window_used_scenarios.append(selected_scenario)

                # ステップ3: 選択したシナリオから質問生成
                print(f"[DEBUG] ステップ2: 質問生成開始...")
                question_start_time = time.time()

                faq = self._generate_question_from_scenario(
                    scenario=selected_scenario,
                    answer_window=window_pair['answer_text'],
                    category=category,
                    used_questions=unique_questions
                )

                question_time = time.time() - question_start_time
                print(f"[TIME] 質問生成時間: {question_time:.1f}秒")

                total_api_time = scenario_time + question_time
