...
**ステップ0: トピック分析（重要）**
まず「質問生成範囲」を読み、異なるトピック・視点を特定してください：
- この範囲には複数の異なる情報が含まれています
- 各文・各段落が異なるトピックを扱っています
- 既に生成された質問とは**完全に異なるトピック**を選んでください
- 同じトピックでも、異なる質問タイプ（「何ですか」「どうなりますか」「どこですか」「誰が」「いつ」など）を使い分けてください
...
**質問バリエーション戦略（必須）:**
この範囲から生成可能な質問タイプ例：
1. 条件・状況に関する質問（「〜の場合、どうなりますか？」）
2. 手続き・プロセスに関する質問（「どのように〜されますか？」）
3. 主体・責任に関する質問（「誰が〜しますか？」「いつ〜しますか？」）
4. 結果・影響に関する質問（「〜するとどうなりますか？」）
5. 対処法に関する質問（「〜の場合、どうすればよいですか？」）
6. 場所・方法に関する質問（「どこに〜すればよいですか？」）
7. 理由・背景に関する質問（「なぜ〜なのですか？」）

**絶対厳守事項:**
- 既に生成された質問（上記の★既存質問や重複判定された質問）と同じトピックは避ける
- 同じトピックでも質問タイプ（疑問詞や焦点）を完全に変える
- 質問生成範囲の**異なる文・異なる段落**から情報を選ぶ
