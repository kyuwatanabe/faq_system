...
**絶対厳守**: 生成する{num_questions}個のFAQは、互いに全く異なるトピックでなければなりません。同じトピックについて複数の質問を生成することは絶対に避けてください。

**重要**: PDFドキュメント全体から均等にトピックを選んでください。特定のトピック（例: ビザウェーバープログラム）だけでなく、PDFに含まれる全てのセクション（ビザの基礎、有効期限と滞在期限、I-94、オーバーステイ、商用と就労の違い、Automatic Revalidationなど）から質問を生成してください。各セクションから最大1つまでです。
...
**絶対厳守**: 生成する{needed}個のFAQは、互いに全く異なるトピックでなければなりません。同じトピックについて複数の質問を生成することは絶対に避けてください。
