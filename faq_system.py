...
3. **最重要**: 上記の既存質問リストを一つ一つ確認し、それらと意味が重複する質問は絶対に生成しないこと
   例: 既存に「アメリカのビザにはどのような種類がありますか?」があれば、「アメリカのビザはどのように分類されていますか?」は生成しない（意味が同じため）
   例: 既存に「ビザの有効期限と滞在期限の違いは何ですか?」があれば、「ビザの有効期間と滞在期間の違いは何ですか?」は生成しない（ほぼ同じ内容のため）
4. 既存質問と完全に異なるトピックや、明確に異なる側面を扱う質問を生成すること
5. 実用的で具体的な質問を作成する
6. 日本人がよく聞きそうな質問にする
7. 回答は正確で詳細、かつ分かりやすい日本語で
8. 専門用語には適切な説明を加える
9. PDFに十分な情報がない場合や、既存質問で主要なトピックがカバーされている場合は、生成する数を大幅に減らしても構わない（例: 要求が10個でも3個だけ生成するなど）
